...
Approved production generation `20260909T170315Z-091d2190` used this path. The source/cold-copy pre-registration SHA-256 values match, and the 6,344,652,800-byte archive independently passed SHA-256, gzip and full listing checks. Retained clone `FullCapture-Debian4-091d2190` and its VHDX remain preserved. An external actor repeatedly restarted Debian4 after the exclusive capture handle was released; this did not affect capture, and corrective final shutdown checks passed, but continuous post-capture stopped state is not claimed.
...
Stop the recovery distro after verification. Keep Debian4 stopped, and retain the archive, manifest, imported distro, failed generations, fixture distros, and all prior backups/evidence. Cleanup, source restart, and retirement remain separate decisions.
