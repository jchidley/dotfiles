...
3. Disposable elevated VHDX attach/guest-read-only capture/restore/detach passed on 8 September; see STATUS. Complete the production coordinator around `capture-offline-system`, including Windows-side exclusive read-only source handling. Obtain concrete approval for the helper environment, real-source downtime and final state. The old freeze controller is disabled; do not retry it.
4. Preserved-pair file restoration passed from the real external repository, including Restic verification of home. See STATUS for exact IDs and retained root. Complete manifest validation and the new plain-archive paired workflow, then implement and verify network-isolated import/boot before starting the recovered system. Disposable VHDX attach/capture/restore/detach passed; production exclusive-source orchestration and boot remain unvalidated. Preserve staged recovery data; exclude temporary/staging contents from the plain system archive.
